...
class Point(object):
    def __init__(self, x, y):
        ...
        self.start_x = x
        self.start_y = y
        self.move_by_total = 0
        self.move_to_total = 0
    ...
    def move_to(self, x, y):
        ...
        endx = self.x
        endy = self.y
        q = ma.sqrt((startx - endx) ** 2 + (starty - endy) ** 2)
        if q < 0:
            q *= -1
        self.move_to_total += q
    ...
    def move_by(self, x, y):
        ...
        if q < 0:
            q *= -1
        self.move_by_total += q

    def get_number_of_moves_made(self):
        return self.total

    def get_distance_from(self, another_point):
        x = ma.sqrt((another_point.x - self.x)**2 + (another_point.y - self.y)**2)
        if x < 0:
            x *= -1
        return x

    def get_distance_from_start(self):
        x = ma.sqrt((self.start_x - self.x)**2 + (self.start_y - self.y)**2)
        if x < 0:
            x *= -1
        return x

    def get_distance_traveled(self):
        return self.move_by_total + self.move_to_total

    def closer_to(self, p2, p3):
        x = ma.sqrt((self.x - p2.x)**2 + (self.y - p2.y)**2)
        if x < 0:
            x *= -1
        y = ma.sqrt((self.x - p3.x)**2 + (self.y - p3.y)**2)
        if y < 0:
            y *= -1
        if x <= y:
            return p2
    ...
    def halfway_to(self, p2):
        x = p2.x - self.x
        y = p2.y - self.y
        return Point((x/2)+self.x, (y/2)+self.y)
